...
export default function HazardsPage() {
  ...
  const [message, setMessage] = useState(''); // أضفناها
  ...
  const fetchHazards = async () => {
    try {
      ...
      if (!response.ok) throw new Error('فشل في تحميل بيانات المخاطر');
  ...
  const handleDelete = async (hazardId: string, hazardName: string) => {
    if (!confirm(`هل أنت متأكد من حذف الخطر "${hazardName}"؟`)) return;
    try {
      const response = await fetch(`/api/hazards/${hazardId}`, { method: 'DELETE' });
      if (!response.ok) throw new Error('فشل في حذف الخطر');
    ...
    <div className="hazards-container">
      ...
      <div className="filters-section">
        <form onSubmit={handleSearch} className="search-form">
          <input type="text" placeholder="ابحث باسم الخطر أو الوصف..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input" />
          ...
          <button type="button" onClick={() => { setSearchTerm(''); setTypeFilter(''); setSeverityFilter(''); setFacilityFilter(''); setCurrentPage(1); }} className="reset-btn">إعادة تعيين</button>
        </form>
      </div>
          ...
          <tbody>
            {hazards.length > 0 ? (
              hazards.map((hazard) => (
                <tr key={hazard.id}>
                  <td>{hazard.name}</td>
                  <td><span className={`type-badge ${hazard.type.toLowerCase()}`}>{getHazardTypeName(hazard.type)}</span></td>
                  <td><span className={`severity-badge ${hazard.severity.toLowerCase()}`}>{getSeverityName(hazard.severity)}</span></td>
                  <td className="description-cell">{hazard.description || 'لا يوجد وصف'}</td>
                  <td>{hazard.facility.name}</td>
                  <td>{hazard.user.name}</td>
                  <td>{new Date(hazard.createdAt).toLocaleDateString('ar-SA')}</td>
                  <td>
                    <div className="action-buttons">
                      <Link href={`/hazards/${hazard.id}/edit`} className="edit-btn">تعديل</Link>
                      <button onClick={() => handleDelete(hazard.id, hazard.name)} className="delete-btn">حذف</button>
                    </div>
                  </td>
                </tr>
              ))
              ...
              <tr>
                <td colSpan={8} className="no-data">لا توجد مخاطر</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {pagination && <div className="total-count">إجمالي المخاطر: {pagination.total}</div>}
    </div>
  );
}
