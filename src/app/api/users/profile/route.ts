...
export async function PUT(request: Request) {
  try {
    ...
    const body: { name?: string; email?: string; currentPassword?: string; newPassword?: string } = await request.json();
    const { name, email, currentPassword, newPassword } = body;

    if (!name || !email) return NextResponse.json({ message: 'الاسم والبريد الإلكتروني مطلوبان' }, { status: 400 });
    ...
    if (email !== user.email) {
      ...
      if (emailExists) return NextResponse.json({ message: 'هذا البريد الإلكتروني مستخدم بالفعل' }, { status: 409 });
    }

    if (newPassword) {
      if (!currentPassword) return NextResponse.json({ message: 'كلمة المرور الحالية مطلوبة' }, { status: 400 });
      const isPasswordValid = await bcrypt.compare(currentPassword, user.password);
      if (!isPasswordValid) return NextResponse.json({ message: 'كلمة المرور الحالية غير صحيحة' }, { status: 400 });
      if (newPassword.length < 6) return NextResponse.json({ message: 'كلمة المرور الجديدة يجب أن تكون على الأقل 6 أحرف' }, { status: 400 });
    }
